...
const AddMenu = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (seletedDishes.length === 0) errors.dishes = true;

        if (hasDiscount) {
            if (!discountPercentage || isNaN(discountPercentage)) {
                errors.discountPercentage = "Invalid";
            } else if (discountPercentage <= 0 || discountPercentage > 100) {
                errors.discountPercentage = "OutOfRange";
            }
        }

        if (Object.keys(errors).length > 0) {
            setFieldErrors(errors);
            showModal("Error", "Please fill all required fields correctly!", "Failed");
            return;
        }
    ...
    return (
        <GestureHandlerRootView>
            <SafeAreaView style={commonStyles.container}>
                <Header title="Create New Menu" showBackButton />
                <ScrollView contentContainerStyle={styles.container}>
                    <TouchableOpacity style={[styles.imagePicker, fieldErrors.image && { borderColor: 'red' }]} onPress={pickImage}>
                        {image ? (
                            <Image
                    ...
                    <TextInput
                        style={[commonStyles.input, fieldErrors.name && { borderColor: 'red' }]}
                    ...
                    <View style={{ gap: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        {hasDiscount && (
                            <TextInput
                                ...
                                style={[commonStyles.input, { width: '30%', textAlign: 'center' }, fieldErrors.discountPercentage && { borderColor: 'red' }
                                ]}
                            />
                        )}
                    </View>
                    ...
                    <View style={styles.modalContent}>
                        <ScrollView style={{ padding: 10 }} contentContainerStyle={{ paddingBottom: 50 }}>
                            {dishes && dishes.map((item) => (
                                <TouchableOpacity
                                    key={item.id}
                                    style={[styles.dishItem, seletedDishes.includes(item.id) && styles.selectedDish]}
